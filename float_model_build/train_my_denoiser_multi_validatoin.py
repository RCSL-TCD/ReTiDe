...
if __name__ == '__main__':
    ...
    test_dataseta = PairedDenoisingDataset("/home/bledc@ad.mee.tcd.ie/data/clement/images/datasets/BSD/noise5/")
    test_datasetb = PairedDenoisingDataset("/home/bledc@ad.mee.tcd.ie/data/clement/images/datasets/BSD/noise20/")
    test_datasetc = PairedDenoisingDataset("/home/bledc@ad.mee.tcd.ie/data/clement/images/datasets/BSD/noise40/")
    ...
    test_loadera = torch.utils.data.DataLoader(
        test_dataseta, batch_size=1,
        shuffle=False, num_workers=8,
        pin_memory=True, drop_last=False)
    
    test_loaderb = torch.utils.data.DataLoader(
        test_datasetb, batch_size=1,
        shuffle=False, num_workers=8,
        pin_memory=True, drop_last=False)
    
    test_loaderc = torch.utils.data.DataLoader(
        test_dataseta, batch_size=1,
        shuffle=False, num_workers=8,
        pin_memory=True, drop_last=False)
    ...
    denoiser_model = UnetGenerator_hardware(3, 3, 4).to(device)
    ...
    for i in range(0, num_epochs + 1):
        ...
        if i % 10 == 0:
            ...
            best, best_epoch = checkpoint_test(test_loadera, model, best, i, best_epoch, text_path, denoiser_model,
                                               device, i, optimizera, experiment_path, train_loss, scheduler)
            best, best_epoch = checkpoint_test(test_loaderb, model, best, i, best_epoch, text_path, denoiser_model,
                                               device, i, optimizera, experiment_path, train_loss, scheduler)
            best, best_epoch = checkpoint_test(test_loaderc, model, best, i, best_epoch, text_path, denoiser_model,
                                               device, i, optimizera, experiment_path, train_loss, scheduler)
