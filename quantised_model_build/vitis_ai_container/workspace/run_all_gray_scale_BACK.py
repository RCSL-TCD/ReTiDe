...
def QAT(model_path, qat_epochs=10):
    ...
    #test_loss, psnr, final_quantized_model = quantize_qat_gray(model, dataset_ntire2_path, batch_size=batch_size, qat_epochs=qat_epochs)
    # model.to(device)
    # # qat_batch_size = 4
    # train_loader, val_loader, test_loader = get_grayscale_loaders(dataset_ntire_path, batch_size=batch_size, val_ratio=0.1, test_ratio=0.3)
    # train_loader2, val_loader2, test_loader2 = get_grayscale_loaders(dataset_ntire_path, batch_size=batch_size, val_ratio=0.1, test_ratio=0.3)
...
if __name__ == "__main__":
    ...
    #test_load(dataset_bsd68_path, unet_f32_weight_path)
    #test_load(dataset_set12_path, unet_f32_weight_path)
    ...
    mse_q, psnr_q, quantized_model_q = QAT(unet_f32_weight_path, qat_epochs=5)

    # benchmark_model(dataset_bsd68_path)
    print(spliter)
    # print(f"Raw Model Metrics:\nMSE: {raw_mse:.4f}, PSNR: {raw_psnr:.2f} dB")
    # print(f"Float32 Model Metrics:\nMSE: {f32_mse:.4f}, PSNR: {f32_psnr:.2f} dB")
    # print(f"PTQ Model Metrics:\nMSE: {mse_p:.4f}, PSNR: {psnr_p:.2f} dB")
