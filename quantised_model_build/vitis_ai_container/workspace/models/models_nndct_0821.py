...
class UnetGenerator_3stage(nn.Module):
    def __init__(self, input_nc, output_nc, base_ch=64):
        ...
        self.add1 = functional.Add()
        self.add2 = functional.Add()
        self.add3 = functional.Add()
        ...
        self.up_relu3 = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        u3 = self.up_relu3(self.up3(b))     # 256ch @ H/4
        #u3 = u3 + e3                       # ADD skip (must match channels)
        u3 = self.add3(u3, e3)  # Residual add
        u3 = self.dec3(u3)                 # 256ch @ H/4

        u2 = self.up_relu2(self.up2(u3))    # 128ch @ H/2
        #u2 = u2 + e2
        u2 = self.add2(u2, e2)  # Residual add
        u2 = self.dec2(u2)                 # 128ch @ H/2

        u1 = self.up_relu1(self.up1(u2))    # 64ch @ H
        #u1 = u1 + e1
        u1 = self.add1(u1, e1)  # Residual add
