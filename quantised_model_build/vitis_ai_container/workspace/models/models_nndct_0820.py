...
class ConvBlock3(nn.Module):
    def __init__(self, in_ch, out_ch, negative_slope=0.1015625):
        ...
        self.act = nn.ReLU(inplace=True)  # Use ReLU for compatibility with hardware
        ...
        self.myidentity = nn.Identity()
        #self.onebyoneconv = nn.Conv2d(out_ch, out_ch, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        y = self.add(y, identity)  # residual add (using functional for compatibility)
        # try if this problem
        #y = self.onebyoneconv(y)  # Identity layer to ensure output shape matches input
...
class UnetGenerator_3stage(nn.Module):
    ...
    def forward(self, x):
        x = self.quant_stub(x)  # Quantize input
        # Encoder
